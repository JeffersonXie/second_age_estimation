...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr  3 10:57:29 2019
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if mode=='ordinal_regression':
                batch_mae,batch_acc,batch_pred_ages=argmax_mae_acc_1(output.cpu(), target)
            if mode=='EL_2group':
...
def demo(data_root, test_list, save, batch_size=64, seed=None):
    ...
    if os.path.exists(os.path.join(save, 'ordinal_regression_resnet18_nesterov_results_test_1_4.csv')):
        os.remove(os.path.join(save, 'ordinal_regression_resnet18_nesterov_results_test_1_4.csv'))
    with open(os.path.join(save, 'ordinal_regression_resnet18_nesterov_results_test_1_4.csv'), 'w') as f:
        f.write('******************************************************************\n')
...
######################fold one#######################################
    ...
    if os.path.exists(model_state_dir_1):                   
        ...
        with open(os.path.join(save, 'ordinal_regression_resnet18_nesterov_results_test_1_4.csv'), 'a') as f:
