...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul  5 11:17:02 2019
...
from ensemble_learning_resnet_two_group_FG_NET import el_resnet101
...
#            print(res)
    ...
    return batch_time.avg, total_loss.avg, argmax_MAE.avg, accuracy.avg
...
#            ARGMAX_MAE,acc=argmax_mae_acc(output.cpu(), target)
            ...
            batch_AE=torch.abs(batch_pred_ages-target[:,100])
...
def argmax_mae_acc(output, target):
    ...
    with torch.no_grad():
        ...
        mae=torch.sum(torch.abs(predicted_ages-target[:,100])).float()/batch_size
...
def demo(data_root, save, n_epochs=1,
    ...
    if os.path.exists(os.path.join(save, 'el_resnet101_nesterov_results_1.csv')):
        os.remove(os.path.join(save, 'el_resnet101_nesterov_results_1.csv'))
    with open(os.path.join(save, 'el_resnet101_nesterov_results_1.csv'), 'w') as f:
        f.write('******************************************************************\n')
        f.write('records on FG_NET dataset under ''leave one person out'' protocol\n\n')
        f.write('******************************************************************\n')
        f.write('\n')
        f.write('\n')
    ...
    for i in range(1, 83):
        train_list='./'+str(i)+'/modified_train.txt'
        test_list='./'+str(i)+'/modified_test.txt'
        ...
        with open(os.path.join(save, 'el_resnet101_nesterov_results_1.csv'), 'a') as f:
            f.write('******************************************************************\n')
            f.write('train and test on ID: {0}\n'.format(i))
            f.write('******************************************************************\n')
        ...
        best_argmax_MAE = 100
        model_state_name_1='el_resnet101_nesterov_model_on_ID_{0}.dat'.format(i)
        ...
        for epoch in range(n_epochs):    
            ...
            with open(os.path.join(save, 'el_resnet101_nesterov_results_1.csv'), 'a') as f:
                f.write('%03d, %0.4f, %0.4f, %0.4f\n'
                        % ((epoch + 1), train_total_loss, train_argmax_MAE, train_accuracy))
        ...
        if os.path.exists(model_state_dir_1):                   
            ...
            AE_lists.extend(AE_list)
            predict_age_lists.extend(predict_age_list)
            real_age_lists.extend(real_age_list) 
            test_MAE_all+=test_argmax_MAE
    ...
    CS_1=CS_1_numerator/len(AE_lists)
    CS_2=CS_2_numerator/len(AE_lists)
    CS_3=CS_3_numerator/len(AE_lists)
    CS_4=CS_4_numerator/len(AE_lists)
    CS_5=CS_5_numerator/len(AE_lists)
    CS_6=CS_6_numerator/len(AE_lists)
    CS_7=CS_7_numerator/len(AE_lists)
    CS_8=CS_8_numerator/len(AE_lists)
    CS_9=CS_9_numerator/len(AE_lists)
    CS_10=CS_10_numerator/len(AE_lists)


    index=0
    with open(os.path.join(save, 'el_resnet101_nesterov_results_1.csv'), 'a') as f:
        f.write('***********************************\n')
        f.write('estimation resuts on FG-NET dataset\n')
        f.write('***********************************\n')
        f.write('\n')
        f.write('test_img_name, real_age, predict_age, absolute_error(AE):\n')
    for i in range(1, 83):
        test_list='./'+str(i)+'/test.txt'
        with open(test_list) as f:
            test_lines=f.readlines()
        for test_line in test_lines:
            img_name=test_line.split()[0]
            img_predict_age=predict_age_lists[index].item()
            img_real_age=real_age_lists[index].item() 
            img_AE=AE_lists[index].item()
            record_line=img_name+'  '+str(round(img_real_age,2))+'  '+str(round(img_predict_age,2))+'  '+str(round(img_AE,2))+'\n'
            index+=1
            with open(os.path.join(save, 'el_resnet101_nesterov_results_1.csv'), 'a') as f:   
                f.write(record_line)
                
    MAE=test_MAE_all/82.0 

    
    with open(os.path.join(save, 'el_resnet101_nesterov_results_1.csv'), 'a') as f:
        f.write('\n')
        f.write('test_MAE: %0.3f\n' % MAE)
