...
# -*- coding: utf-8 -*-
"""
Created on Sat Mar 30 14:34:35 2019
...
#        EXP_MAE=exp_MAE(output.cpu(), target)
        ...
        kl_loss_term=KLloss(new_output, target_var[:,0:102])/batch_size

        
        cross_entropy_loss_term=F.cross_entropy(output, target_var[:,102].long())
        ...
        l1_loss_term=L1_loss(batches_predict_age, target_var[:,102])
...
#            MAE=exp_MAE(output.cpu(), target)            
            ...
            batch_AE=torch.abs(batches_predict_age-target_var[:,102])
            AE_list.extend(batch_AE.cpu())
            predict_age_list.extend(batches_predict_age.cpu())
            real_age_list.extend(target[:,102])
...
def exp_MAE(batches_predict_age, target):
    ...
    with torch.no_grad():
        batch_size = target.size(0)          
        absolute_error_all=torch.sum(torch.abs(batches_predict_age-target[:,102]))
...
#      batch_size=64, lr=0.001, wd=0.0005, seed=None):
    ...
    if os.path.exists(os.path.join(save, 'DLDL_expectation_ResNet18_nesterov_results_3.csv')):
        os.remove(os.path.join(save, 'DLDL_expectation_ResNet18_nesterov_results_3.csv'))
    with open(os.path.join(save, 'DLDL_expectation_ResNet18_nesterov_results_3.csv'), 'w') as f:
        f.write('******************************************************************\n')
...
#                                                         gamma=0.1)
    ...
    best_exp_MAE = 100
    model_state_name='DLDL_expection_resnet18_nesterov_model_3.dat'
    model_state_dir=os.path.join(save, model_state_name)
    
    with open(os.path.join(save, 'DLDL_expectation_ResNet18_nesterov_results_3.csv'), 'a') as f:
