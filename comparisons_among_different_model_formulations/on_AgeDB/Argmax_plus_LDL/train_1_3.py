...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 27 11:13:30 2019
...
def train(model, loader, optimizer, epoch, n_epochs, device, print_freq=1):
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        loss=KLloss(new_output, target_var[:,0:102])/batch_size
...
#                'Loss %.4f (%.4f)' % (train_loss.val, train_loss.avg),
...
#            print(res)
    ...
    return batch_time.avg, train_loss.avg, train_argmax_MAE.avg, train_exp_MAE.avg
...
#            os._exit(0)
            ...
            validate_loss.update(loss.item(), batch_size)
...
#                    'Iter: [%d/%d]' % (batch_idx + 1, len(loader)),
#                    'Time %.3f (%.3f)' % (batch_time.val, batch_time.avg),
#                    'Loss %.4f (%.4f)' % (validate_loss.val, validate_loss.avg),
...
#                print(res)
        ...
        # Return summary statistics
        return batch_time.avg, validate_loss.avg, validate_argmax_MAE.avg, validate_exp_MAE.avg
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if MAE_mode=='argmax':
                ...
                batch_AE=torch.abs(batches_predict_age.float()-target_var[:,102])
...
#                    'Iter: [%d/%d]' % (batch_idx + 1, len(loader)),
#                    'Time %.3f (%.3f)' % (batch_time.val, batch_time.avg),
#                    'Loss %.4f (%.4f)' % (test_loss.val, test_loss.avg),
#                    'test_MAE %.4f (%.4f)' % (test_MAE.val, test_MAE.avg),
#                ])
#                print(res)

        # Return summary statistics
        return batch_time.avg, test_loss.avg, test_MAE.avg, AE_list, predict_age_list, real_age_list
...
def argmax_MAE(output, target):
    ...
    with torch.no_grad():
        ...
        pred_age_prob=F.softmax(output[:,0:102],1)
        batches_predict_age=torch.argmax(pred_age_prob, dim=1)          
        absolute_error_all=torch.sum(torch.abs(batches_predict_age.float()-target[:,102]))
...
def demo(data_root, train_list, validate_list, test_list, save, n_epochs=1,
    ...
    if os.path.exists(os.path.join(save, 'DLDL_ResNet18_nesterov_results_train_1_3.csv')):
        os.remove(os.path.join(save, 'DLDL_ResNet18_nesterov_results_train_1_3.csv'))
    with open(os.path.join(save, 'DLDL_ResNet18_nesterov_results_train_1_3.csv'), 'w') as f:
        f.write('******************************************************************\n')
...
#    # Train and validate model
    best_argmax_MAE = 100
    best_exp_MAE = 100
    model_state_name_1='DLDL_resnet18_nesterov_model_train_1_3_1.dat'
    model_state_name_2='DLDL_resnet18_nesterov_model_train_1_3_2.dat'
    model_state_dir_1=os.path.join(save, model_state_name_1)
    model_state_dir_2=os.path.join(save, model_state_name_2)
    
    with open(os.path.join(save, 'DLDL_ResNet18_nesterov_results_train_1_3.csv'), 'a') as f:
