...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 26 20:37:47 2019
...
def train(model, loader, optimizer, epoch, n_epochs, device, print_freq=1):
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        L1_loss=nn.L1Loss()
        l1_loss_term=L1_loss(output.squeeze(dim=1), target_var[:,0])
        
        loss=l1_loss_term

        l1_loss.update(l1_loss_term.item(),batch_size)
        train_MAE.update(l1_loss_term.item(), batch_size)
...
def validate(model, loader, epoch, n_epochs, device, print_freq=1):
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            l1_loss.update(l1_loss_term.item(),batch_size)
            validate_MAE.update(l1_loss_term.item(), batch_size)
...
def test(model, loader, device, model_state_dir, print_freq=1): 
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            batches_predict_age=output.squeeze(dim=1) 
            ...
            l1_loss.update(l1_loss_term.item(), batch_size)
            test_MAE.update(l1_loss_term.item(), batch_size)
...
#      batch_size=64, lr=0.001, wd=0.0005, seed=None):
    ...
    if os.path.exists(os.path.join(save, 'regression_mae_ResNet18_nesterov_results_train_1_3.csv')):
        os.remove(os.path.join(save, 'regression_mae_ResNet18_nesterov_results_train_1_3.csv'))
    with open(os.path.join(save, 'regression_mae_ResNet18_nesterov_results_train_1_3.csv'), 'w') as f:
        f.write('******************************************************************\n')
...
#                                                     gamma=0.1)
    ...
    best_MAE = 100
    model_state_name_1='regression_mae_resnet18_nesterov_model_train_1_3.dat'
    model_state_dir_1=os.path.join(save, model_state_name_1)
    
    with open(os.path.join(save, 'regression_mae_ResNet18_nesterov_results_train_1_3.csv'), 'a') as f:
