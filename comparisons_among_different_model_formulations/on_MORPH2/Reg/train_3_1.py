...
# -*- coding: utf-8 -*-
"""
Created on Sun Mar 24 19:55:55 2019
...
#    train_round_MAE=AverageMeter()
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        L1_loss=nn.L1Loss()
        l1_loss_term=L1_loss(output.squeeze(), target_var[:,101])
        
        loss=l1_loss_term
        batch_mae=MAE(output.cpu(), target)
#        batch_round_mae=round_MAE(output.cpu(), target)

        l1_loss.update(l1_loss_term.item(),batch_size)
        train_MAE.update(batch_mae.item(), batch_size)
#        train_round_MAE.update(batch_round_mae.item(), batch_size)
...
#            print(res)
    ...
    return batch_time.avg, l1_loss.avg, train_MAE.avg
...
def validate(model, loader, epoch, n_epochs, device, print_freq=1):
    ...
    batch_time = AverageMeter()
    l1_loss=AverageMeter()  
    validate_MAE=AverageMeter()
#    validate_round_MAE=AverageMeter()
    # Model on train mode
    model.eval()
...
#            batch_round_mae=round_MAE(output.cpu(), target)
            ...
            l1_loss.update(l1_loss_term.item(),batch_size)
            validate_MAE.update(batch_mae.item(), batch_size)
#            validate_round_MAE.update(batch_round_mae.item(), batch_size)    
    ...
    #            print(res)
        ...
        # Return summary statistics
        return batch_time.avg, l1_loss.avg, validate_MAE.avg
...
def test(model, loader, device, model_state_dir, print_freq=1): 
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            batch_mae=MAE(output.cpu(), target)
            batches_predict_age=output.squeeze()
            ...
            l1_loss.update(l1_loss_term.item(), batch_size)
            test_MAE.update(batch_mae.item(), batch_size)
...
#                print(res)
        ...
        # Return summary statistics
        return batch_time.avg, l1_loss.avg, test_MAE.avg, AE_list, predict_age_list, real_age_list


def MAE(output, target):
    with torch.no_grad():
        batch_size = target.size(0)  
        batch_pred_ages=output.squeeze()       
        absolute_error_all=torch.sum(torch.abs(batch_pred_ages-target[:,101]))
...
#      batch_size=64, lr=0.001, wd=0.0005, seed=None):
    ...
    if os.path.exists(os.path.join(save, 'regression_mae_ResNet18_nesterov_results_3_1.csv')):
        os.remove(os.path.join(save, 'regression_mae_ResNet18_nesterov_results_3_1.csv'))
    with open(os.path.join(save, 'regression_mae_ResNet18_nesterov_results_3_1.csv'), 'w') as f:
        f.write('******************************************************************\n')
...
#                                                     gamma=0.1)
    ...
    best_MAE = 100
    model_state_name_1='regression_mae_resnet18_nesterov_model_3_1.dat'
    model_state_dir_1=os.path.join(save, model_state_name_1)

    
    with open(os.path.join(save, 'regression_mae_ResNet18_nesterov_results_3_1.csv'), 'a') as f:
