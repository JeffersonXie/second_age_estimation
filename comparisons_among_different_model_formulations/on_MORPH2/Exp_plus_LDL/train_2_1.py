...
# -*- coding: utf-8 -*-
"""
Created on Sat Mar 30 12:36:35 2019
...
#        EXP_MAE=exp_MAE(output.cpu(), target)
        ...
        l1_loss_term=L1_loss(batches_predict_age, target_var[:,101])
        
    
        loss=l1_loss_term+kl_loss_term

        EXP_MAE=exp_MAE(batches_predict_age.cpu(), target)

        l1_loss.update(l1_loss_term.item(), batch_size)
...
#            print(res)
    ...
    return batch_time.avg, total_loss.avg, l1_loss.avg, cross_entropy_loss.avg, kl_loss.avg, train_exp_MAE.avg
...
#                print(res)
        ...
        # Return summary statistics
        return batch_time.avg, total_loss.avg, l1_loss.avg, cross_entropy_loss.avg, kl_loss.avg, validate_exp_MAE.avg
...
#            MAE=exp_MAE(output.cpu(), target)            
            ...
            MAE=exp_MAE(batches_predict_age.cpu(), target)
...
#                print(res)
        ...
        # Return summary statistics
        return batch_time.avg, total_loss.avg, l1_loss.avg, cross_entropy_loss.avg, kl_loss.avg, test_MAE.avg, AE_list, predict_age_list, real_age_list



#def exp_MAE(output, target):
def exp_MAE(batches_predict_age, target):
...
#      batch_size=64, lr=0.001, wd=0.0005, seed=None):
    ...
    if os.path.exists(os.path.join(save, 'DLDL_expectation_ResNet18_nesterov_results_2_1.csv')):
        os.remove(os.path.join(save, 'DLDL_expectation_ResNet18_nesterov_results_2_1.csv'))
    with open(os.path.join(save, 'DLDL_expectation_ResNet18_nesterov_results_2_1.csv'), 'w') as f:
        f.write('******************************************************************\n')
...
#                                                         gamma=0.1)
    ...
    best_exp_MAE = 100
    model_state_name='DLDL_expection_resnet18_nesterov_model_2_1.dat'
    model_state_dir=os.path.join(save, model_state_name)
    
    with open(os.path.join(save, 'DLDL_expectation_ResNet18_nesterov_results_2_1.csv'), 'a') as f:
#            f.write('epoch,train_loss,train_male_age_accuracy,train_female_age_accuracy,train_gender_accuracy,train_MAE\n')
        f.write('epoch, train_loss, train_l1_loss, train_cross_entropy_loss, train_kl_loss, train_exp_MAE, valid_loss, valid_l1_loss, valid_cross_entropy_loss, valid_kl_loss, valid_exp_MAE\n')

    for epoch in range(n_epochs):
        
        scheduler.step()
        _, train_loss, train_l1_loss, train_cross_entropy_loss, train_kl_loss, train_exp_MAE = train(
            model=model_wrapper,
            loader=train_loader,
            optimizer=optimizer,
            epoch=epoch,
        ...
        _, valid_loss, valid_l1_loss, valid_cross_entropy_loss, valid_kl_loss, valid_exp_MAE = validate(
            model=model_wrapper,
            loader=valid_loader,
            epoch=epoch,
            n_epochs=n_epochs,
...
#         Determine if model is the best
        ...
        with open(os.path.join(save, 'DLDL_expectation_ResNet18_nesterov_results_2_1.csv'), 'a') as f:
            f.write('%03d, %0.4f, %0.4f, %0.4f, %0.4f, %0.3f, %0.4f, %0.4f, %0.4f, %0.4f, %0.3f\n' 
                    % ((epoch + 1), train_loss, train_l1_loss, train_cross_entropy_loss, train_kl_loss, train_exp_MAE,
                       valid_loss, valid_l1_loss, valid_cross_entropy_loss, valid_kl_loss, valid_exp_MAE))
        if  math.isnan(float(train_exp_MAE)):
            break
    ...
    if os.path.exists(model_state_dir):                   
        _, test_loss, test_l1_loss, test_cross_entropy_loss, test_kl_loss, test_exp_MAE, AE_list, predict_age_list, real_age_list= test(
            model=model_wrapper,
            loader=test_loader,
            device=device,
        ...
        os.remove(model_state_dir)
        with open(os.path.join(save, 'DLDL_expectation_ResNet18_nesterov_results_2_1.csv'), 'a') as f:
            f.write('test_loss, test_l1_loss, test_cross_entropy_loss, test_kl_loss, test_exp_MAE:\n')
            f.write('%0.4f, %0.4f, %0.4f, %0.4f, %0.3f\n' % (test_loss, test_l1_loss, test_cross_entropy_loss, test_kl_loss, test_exp_MAE))
#            f.write('\n') 
