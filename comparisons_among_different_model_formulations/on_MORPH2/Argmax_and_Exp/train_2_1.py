...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar 25 12:58:15 2019
...
def train(model, loader, optimizer, epoch, n_epochs, device, print_freq=1):
    ...
    train_round_exp_MAE=AverageMeter()
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        ROUND_EXP_MAE=round_exp_MAE(output.cpu(),target)
        
        new_output=F.log_softmax(output,1)
#        KLloss=nn.KLDivLoss()
#        kl_loss_term=KLloss(new_output, target_var[:,0:101])
        KLloss=nn.KLDivLoss(reduction='sum')
        kl_loss_term=KLloss(new_output, target_var[:,0:101])/batch_size

        
        cross_entropy_loss_term=F.cross_entropy(output, target_var[:,101].long())
        
        loss=cross_entropy_loss_term

        
        cross_entropy_loss.update(cross_entropy_loss_term.item(), batch_size)
        kl_loss.update(kl_loss_term.item(), batch_size)
        ...
        train_round_exp_MAE.update(ROUND_EXP_MAE.item(), batch_size)
...
#            print(res)
    ...
    return batch_time.avg, total_loss.avg, cross_entropy_loss.avg, kl_loss.avg, train_argmax_MAE.avg, train_exp_MAE.avg, train_round_exp_MAE.avg
...
def validate(model, loader, epoch, n_epochs, device, print_freq=1):
    ...
    validate_round_exp_MAE=AverageMeter()
    # Model on test mode
    model.eval()
...
#            os._exit(0)
            ...
            EXP_MAE=exp_MAE(output.cpu(), target) 
            ROUND_EXP_MAE=round_exp_MAE(output.cpu(),target)            
#            loss=F.cross_entropy(output[:,0:101], target_var[:,0])
            new_output=F.log_softmax(output,1)
            KLloss=nn.KLDivLoss(reduction='sum')
            kl_loss_term=KLloss(new_output, target_var[:,0:101])/batch_size
#            KLloss=nn.KLDivLoss()
#            kl_loss_term=KLloss(new_output, target_var[:,0:101])
            ...
            validate_round_exp_MAE.update(ROUND_EXP_MAE.item(), batch_size)
...
#                print(res)
        ...
        # Return summary statistics
        return batch_time.avg, total_loss.avg, cross_entropy_loss.avg, kl_loss.avg, validate_argmax_MAE.avg, validate_exp_MAE.avg, validate_round_exp_MAE.avg
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if MAE_mode=='argmax':
                ...
                pred_age_prob=F.softmax(output[:,0:101],1)
                batches_predict_age=torch.argmax(pred_age_prob, dim=1)  
                
                batch_AE=torch.abs(batches_predict_age.float()-target_var[:,101])
                ...
                batch_AE=torch.abs(batches_predict_age-target_var[:,101])
                AE_list.extend(batch_AE.cpu())
                predict_age_list.extend(batches_predict_age.cpu())
                real_age_list.extend(target[:,101])
            else:
                MAE=round_exp_MAE(output.cpu(), target)            
                ...
                pred_age_prob=F.softmax(output[:,0:101],1)
                batches_predict_age=torch.sum(age_list_var*pred_age_prob,dim=1) 
                new_batches_predict_age=torch.round(batches_predict_age)
                
                batch_AE=torch.abs(new_batches_predict_age-target_var[:,101])
                AE_list.extend(batch_AE.cpu())
                predict_age_list.extend(new_batches_predict_age.cpu())
                real_age_list.extend(target[:,101])                
...
#                print(res)
        ...
        # Return summary statistics
        return batch_time.avg, total_loss.avg, cross_entropy_loss.avg, kl_loss.avg, test_MAE.avg, AE_list, predict_age_list, real_age_list
...
def argmax_MAE(output, target):
    ...
    with torch.no_grad():
        ...
        pred_age_prob=F.softmax(output[:,0:101],1)
        batches_predict_age=torch.argmax(pred_age_prob, dim=1)          
        absolute_error_all=torch.sum(torch.abs(batches_predict_age.float()-target[:,101]))
...
def exp_MAE(output, target):
    ...
    with torch.no_grad():
        ...
        pred_age_prob=F.softmax(output[:,0:101],1)
        batches_predict_age=torch.sum(age_list_var*pred_age_prob,dim=1)          
        absolute_error_all=torch.sum(torch.abs(batches_predict_age-target[:,101]))
        MAE=absolute_error_all/batch_size
        
        return MAE
    
def round_exp_MAE(output, target):
    """Computes the precision@k for the specified values of k"""
    with torch.no_grad():
        batch_size = target.size(0) 
        age_range=np.arange(101).reshape((1,101))
        ...
        pred_age_prob=F.softmax(output[:,0:101],1)
        batches_predict_age=torch.sum(age_list_var*pred_age_prob,dim=1)  
        new_batch_predict_age=torch.round(batches_predict_age)        
#        
#        print(new_batch_predict_age)
#        print(new_batch_predict_age.type())
#        os._exit(0)
        
        absolute_error_all=torch.sum(torch.abs(new_batch_predict_age-target[:,101]))
        MAE=absolute_error_all/batch_size
...
#      batch_size=64, lr=0.001, wd=0.0005, seed=None):
    ...
    if os.path.exists(os.path.join(save, 'argmax_exp_cross_entropy_ResNet18_nesterov_results_2_1.csv')):
        os.remove(os.path.join(save, 'argmax_exp_cross_entropy_ResNet18_nesterov_results_2_1.csv'))
    with open(os.path.join(save, 'argmax_exp_cross_entropy_ResNet18_nesterov_results_2_1.csv'), 'w') as f:
        f.write('******************************************************************\n')
...
#                                                         gamma=0.1)
    ...
    best_argmax_MAE = 100
    best_exp_MAE = 100
    best_round_exp_MAE=100
    model_state_name_1='argmax_resnet18_nesterov_model_2_1_1.dat'
    model_state_name_2='exp_resnet18_nesterov_model_2_1_2.dat'
    model_state_name_3='round_exp_resnet18_nesterov_model_2_1_3.dat'
    ...
    with open(os.path.join(save, 'argmax_exp_cross_entropy_ResNet18_nesterov_results_2_1.csv'), 'a') as f:
#            f.write('epoch,train_loss,train_male_age_accuracy,train_female_age_accuracy,train_gender_accuracy,train_MAE\n')
        f.write('epoch, train_loss, train_cross_entropy_loss, train_kl_loss, train_argmax_MAE, train_exp_MAE, train_round_exp_MAE, valid_loss, valid_cross_entropy_loss, valid_kl_loss, valid_argmax_MAE, valid_exp_MAE, valid_round_exp_MAE\n')

    for epoch in range(n_epochs):
        
        scheduler.step()
        _, train_loss, train_cross_entropy_loss, train_kl_loss, train_argmax_MAE, train_exp_MAE, train_round_exp_MAE = train(
            model=model_wrapper,
            loader=train_loader,
            optimizer=optimizer,
            epoch=epoch,
        ...
        _, valid_loss, valid_cross_entropy_loss, valid_kl_loss, valid_argmax_MAE, valid_exp_MAE, valid_round_exp_MAE = validate(
            model=model_wrapper,
            loader=valid_loader,
            epoch=epoch,
            n_epochs=n_epochs,
...
#         Determine if model is the best
        ...
        if valid_round_exp_MAE < best_round_exp_MAE:
            best_round_exp_MAE=valid_round_exp_MAE
        ...
        with open(os.path.join(save, 'argmax_exp_cross_entropy_ResNet18_nesterov_results_2_1.csv'), 'a') as f:
            f.write('%03d, %0.4f, %0.3f, %0.4f, %0.3f, %0.3f, %0.3f, %0.4f, %0.3f, %0.4f, %0.3f, %0.3f, %0.3f\n' 
                    % ((epoch + 1), train_loss, train_cross_entropy_loss, train_kl_loss, train_argmax_MAE, train_exp_MAE, train_round_exp_MAE,
                       valid_loss, valid_cross_entropy_loss, valid_kl_loss, valid_argmax_MAE, valid_exp_MAE, valid_round_exp_MAE))
...
#            break
    ...
    # Test model       
    if os.path.exists(model_state_dir_1):                   
        _, test_loss, test_cross_entropy_loss, test_kl_loss, test_argmax_MAE, AE_list, predict_age_list, real_age_list= test(
            model=model_wrapper,
            loader=test_loader,
            device=device,
            MAE_mode='argmax',
...
#        os.remove(model_state_dir)
        with open(os.path.join(save, 'argmax_exp_cross_entropy_ResNet18_nesterov_results_2_1.csv'), 'a') as f:
            f.write('test_loss, test_cross_entropy_loss, test_kl_loss, test_argmax_MAE:\n')
            f.write('%0.4f, %0.3f, %0.4f, %0.3f\n' % (test_loss, test_cross_entropy_loss, test_kl_loss, test_argmax_MAE))
#            f.write('\n') 
...
#'***************************'
    ...
    if os.path.exists(model_state_dir_2):                   
        _, test_loss, test_cross_entropy_loss, test_kl_loss, test_exp_MAE, AE_list, predict_age_list, real_age_list= test(
            model=model_wrapper,
            loader=test_loader,
            device=device,
            MAE_mode='exp',
...
#        os.remove(model_state_dir)
        with open(os.path.join(save, 'argmax_exp_cross_entropy_ResNet18_nesterov_results_2_1.csv'), 'a') as f:
            f.write('test_loss, test_cross_entropy_loss, test_kl_loss, test_exp_MAE:\n')
            f.write('%0.4f, %0.3f, %0.4f, %0.3f\n' % (test_loss, test_cross_entropy_loss, test_kl_loss, test_exp_MAE))
#            f.write('\n') 
...
#'***************************'
    ...
    if os.path.exists(model_state_dir_3):                   
        _, test_loss, test_cross_entropy_loss, test_kl_loss, test_round_exp_MAE, AE_list, predict_age_list, real_age_list= test(
            model=model_wrapper,
            loader=test_loader,
            device=device,
            MAE_mode='round_exp',
...
#        os.remove(model_state_dir)
        with open(os.path.join(save, 'argmax_exp_cross_entropy_ResNet18_nesterov_results_2_1.csv'), 'a') as f:
            f.write('test_loss, test_cross_entropy_loss, test_kl_loss, test_round_exp_MAE:\n')
            f.write('%0.4f, %0.3f, %0.4f, %0.3f\n' % (test_loss, test_cross_entropy_loss, test_kl_loss, test_round_exp_MAE))
#            f.write('\n') 
